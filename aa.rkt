...
(define in (get-pure-port penteli #:redirections 5))


(define response-bytes (port->bytes in))
(close-input-port in)
; (struct custom-request (host headers port ssl type))

; (parameterize ([current-locale "C"])  ; C locale supports ASCII,
;    (bytes->string/locale response-bytes))

(define converter  (bytes-open-converter "cp1253" "UTF-8"))
(match-define-values ( a _ _) (bytes-convert converter response-bytes))
(bytes-close-converter converter)
(define poutsa (html->xexp a))
(define qu (sxpath "//div[contains(@class, 'content_values')]/div[contains(@class, 'col left_col')]/div[contains(@class, 'col_sub dist boxshadow')]/div[contains(@class, 'line')]/div[contains(@class, 'lright')]/text()"))
